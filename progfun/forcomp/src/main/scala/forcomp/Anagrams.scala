...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toList.groupBy(_.toLower).map { case (x, y) => (x, y.length) }.toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentences(occ: Occurrences, acc: List[Word]): List[Sentence] = occ match {
      case Nil => List(acc)
      case _ => for {
        combo <- combinations(occ)
        word <- dictionaryByOccurrences.getOrElse(combo, Nil)
        if word != Nil
        sentence <- sentences(subtract(occ, combo), acc ++ List(word))
      } yield sentence
    }
    sentences(sentenceOccurrences(sentence), Nil)
  }
}