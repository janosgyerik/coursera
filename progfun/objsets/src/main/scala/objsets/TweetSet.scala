...
abstract class TweetSet {
   ...
   * Question: Should we implment this method here, or should it remain abstract
   * and be implemented in the subclasses?
   * Answer: cannot implement here, depends on the underlying data model.
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def max(branch: TweetSet, tweet: Tweet) = {
      if (branch.isEmpty) tweet
      else {
        val bmost = branch.mostRetweeted
    	if (bmost.retweets > tweet.retweets) bmost else tweet
      }
    }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets filter(x => google.exists(s => x.text.contains(s)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets filter(x => apple.exists(s => x.text.contains(s)))
