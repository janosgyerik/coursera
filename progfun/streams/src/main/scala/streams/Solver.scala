...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
      ...
      case Stream() => Stream()
      case (b, hist) #:: xs =>
        if (explored.contains(b)) newNeighborsOnly(xs, explored)
        else (b, hist) #:: newNeighborsOnly(xs, explored)
    }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val next = initial.flatMap { case (b, history) => newNeighborsOnly(neighborsWithHistory(b, history), explored + b) }
    if (next.isEmpty) next
    else next ++ from(next, explored ++ next.map { case (b, _) => b })
  }
